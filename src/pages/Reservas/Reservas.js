...
const Reservas  = () => {
  const [comensal, setComensal] = React.useState('');
  const handleChangeComensal = (event: SelectChangeEvent) => {
    setComensal(event.target.value);
  };

  const [restoran, setRestoran] = React.useState('');
  const handleChangeRestoran = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
        ...
        <Grid container spacing={{ xs: 1, md: 4, lg: 5 }} columns={{ xs: 1, sm: 1, md: 2, lg: 2 }} display="flex" justifyContent="center">
            ...
            <Grid item >
                <Box component="form" sx={{ '& > :not(style)': { m: 1, width: '50ch', marginTop: 2,}   }}   noValidate autoComplete="off">
                    <TextField id="nombre" label="Nombre" variant="outlined" />
                </Box>
                <Box component="form" sx={{ '& > :not(style)': { m: 1, width: '24ch' },  }}   noValidate autoComplete="off">
                ...
                <Box component="form" sx={{ '& > :not(style)': { m: 1, width: '40ch',  mt: 1 }   }}   noValidate autoComplete="off">
                    <Button variant="contained" startIcon={<SaveIcon />}
                                      href="/"> Confirma Reserva
                    </Button>
                </Box>
            </Grid>
        </Grid>
